...
function	ping_Usermanagement() {
	axios.get(`https://${window.IP}:8000/auth/health/`)
	.then((response) => {
...
function	update_user_info() {
	...
	axios.get(`https://${window.IP}:8000/auth/users/`, {headers: userheaders})
	.then((response) => {
...
function	get_all_users() {
	...
	axios.get(`https://${window.IP}:8000/auth/users/all/`, {headers: userheaders})
	.then((response) => {
...
function	get_user_info(target) {
	...
	axios.get(`https://${window.IP}:8000/auth/users/${target}/`, {headers: userheaders})
	.then((response) => {
		return response.data
	})
	.catch((error) => {
